...
class MapDetailViewController: UIViewController, GMSMapViewDelegate {
  ...
  init(trip: TripDetail) {
    self.trip = trip
    print("[MapDetailViewController] trip", trip.photos)
    let testLocationDetails = trip.photos.map{ LocationDetailMarker(latitude: $0.photoLocation?.latitude ??  42.0, longitude: $0.photoLocation?.longitude ?? 42.0) }
